...
func (bb *ByteBuffer) MustReadAt(p []byte, offset int64) {
	if offset < 0 {
		log.Panicf("BUG: cannot read at negative offset=%d", offset)
	}
	if offset > int64(len(bb.B)) {
		log.Panicf("BUG: too big offset=%d; cannot exceed len(bb.B)=%d", offset, len(bb.B))
	}
	if n := copy(p, bb.B[offset:]); n < len(p) {
		log.Panicf("BUG: EOF occurred after reading %d bytes out of %d bytes at offset %d", n, len(p), offset)
	}
}
...
func (bb *ByteBuffer) NewReader() io.ReadCloser {
	return &reader{
		bb: bb,
	}
}
...
// Close implements the io.ReadCloser.
